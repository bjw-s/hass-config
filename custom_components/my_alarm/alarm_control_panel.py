...
CONF_PERSONAL_CODES = "personal_codes"
...
ATTR_LAST_CODE_ENTERED = "last_code_entered"
...
PLATFORM_SCHEMA = vol.Schema(
    vol.All(
        {
            vol.Required(CONF_PLATFORM): "my_alarm",
            ...
            vol.Optional(
                CONF_PERSONAL_CODES, default=[]): vol.All(
...
def setup_platform(hass, config, add_entities, discovery_info=None):
    ...
    if config.get(CONF_CODE):
        codes.append(config.get(CONF_CODE))

    if config.get(CONF_PERSONAL_CODES):
        for item in config.get(CONF_PERSONAL_CODES):
            codes.append(item[CONF_CODE])

    for possible_duplicate_code in codes:
        if possible_duplicate_code in clean_codes_set:
            _LOGGER.error('Duplicate code detected, please check config.')
            return False
        else:
            clean_codes_set.add(possible_duplicate_code)
    ...
    add_entities(
        [
            ManualAlarm(
                ...
                config.get(CONF_PERSONAL_CODES),
...
class ManualAlarm(alarm.AlarmControlPanelEntity, RestoreEntity):
    ...
    def __init__(
        ...
        self._personal_codes = None
        if personal_codes:
            self._personal_codes = personal_codes
        self._last_code_entered = None
    ...
    def _validate_code(self, code, state):
        """Validate given code."""
        if self._personal_codes:
            result = [item for item in self._personal_codes if item['code'] == code]
            if len(result) > 0:
                self._last_code_entered = result[0]['name']
                return True
        ...
        self._last_code_entered = DEFAULT_LAST_CODE
        return check
    ...
    def device_state_attributes(self):
        ...
        if self._last_code_entered:
            state_attr[ATTR_LAST_CODE_ENTERED] = self._last_code_entered
